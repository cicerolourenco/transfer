...
class Utils
{
	...
	public static $tipos_sangue = array('A+','A-','B+','B-','AB+','AB-','O+','O-');
	public static $vetor_meses = array("Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro");
	public static $vetor_semana = array("Domingo", "Segunda-Feira", "Terça-Feira", "Quarta-Feira", "Quinta-Feira", "Sexta-Feira", "Sábado");
	...
	/**
	 * Transforma um texto com quebras de linha numa lista não ordenada
	 * @param string $texto Texto a ser transformado
	 */
	public static function parse_br_to_list($texto, $classe='')
	{
		if($texto!='')
		{
			$texto=nl2br($texto);
			$retorno = $classe=='' ? '<ul>' : '<ul class="' . $classe . '">';
	...
	/**
	 * Formata um preço com os separadores e o arredondamento necessários
	 * @param float $num Preço
	 * @param int $casas Quantidade de casas decimais
	 */
	public static function formata_preco($num, $casas=2) 
	{
		$num_fmt = number_format(round($num, $casas), $casas, ",", ".");
		return $num_fmt;
	}	
	...
	{
		return preg_replace(self::REGEX_URL_LIMPA,'',self::remove_acentos(strtolower(str_replace(' ', '-', trim($string)))));
	} 
	...
	public static function formata_cpf($cpf)
	{
		...
		return $cpf=='' ? '' : $cpf_numeros . "-" . $cpf_digitos;
	}
	...
	public static function formata_cnpj($cnpj)
	{
		...
		return $cnpj=='' ? '' : $cnpj_bloco1 . "." . $cnpj_bloco2 . "." . $cnpj_bloco3 . "/" . $cnpj_bloco4 . "-" . $cnpj_bloco5;
	}

	
	/**
	 * Devolve um CEP numérico com a separação entre os 5 primeiros e os 3 últimos dígitos
	 * @param string $cep CEP numérico (apenas dígitos) 
	 */
	public static function formata_cep($cep)
	{
		$cep = preg_replace( '/[^0-9]/', '', $cep);
		return $cep=='' ? '' : substr($cep, 0, 5) . "-" . substr($cep, 5);
	}
	
	
	/**
	 * Formata a string do telefone
	 */
	public static function formata_tel($tel)
	{
		...
		{
			case 11:
    			$tel_fmt = '(' . substr($tel, 0, 2) . ') ' . substr($tel, 2, 5) . '-' . substr($tel, 7);
			break;
			
			case 10:
    			$tel_fmt = '(' . substr($tel, 0, 2) . ') ' . substr($tel, 2, 4) . '-' . substr($tel, 6);
			break;
			
			case 9:
				$tel_fmt = substr($tel, 0, 5) . '-' . substr($tel, 5);
			break;
			
			default:
				$tel_fmt = substr($tel, 0, 4) . '-' . substr($tel, 4);
		}
	...
	/**
	 * Retorna o hash do vídeo no Vimeo. Se retornar null, o endereço é inválido
	 * @param string $url_video URL do vídeo no Vimeo
	 */
	public static function get_hash_vimeo($url_video)
	{
		if (preg_match('/^.*(vimeo\.com\/)((channels\/[A-z]+\/)|(groups\/[A-z]+\/videos\/))?([0-9]+)/', $url_video, $match)) 
		{
	    	$video_id = $match[5];
		}
	...
	/**
	 * Retorna a imagem do Vimeo
	 * @param string $id ID do vídeo
	 * @param string $size Nome do tamanho do thumbnail
	 */
	public static function get_img_vimeo($id, $size='thumbnail_small') 
	{
	...
	/**
	 * Retorna o hash do vídeo no Youtube. Se retornar null, o endereço é inválido
	 * @param string $url_video URL do vídeo no Youtube
	 */
	public static function get_hash_youtube($url_video)
	{
	...
	/**
	 * Retorna a URL do embed do Youtube 
	 * @param string $url_video Endereço completo do vídeo
	 */
	public static function get_path_youtube($url_video)
	{
		return 'http://www.youtube.com/embed/' . self::get_hash_youtube($url_video);
	}
	
	
	/**
	 * Retorna a imagem do vídeo no Youtube
	 * @param string $hash Hash (ID) do vídeo
	 */
	public static function get_img_youtube($hash, $size='2')
	{
		return 'http://img.youtube.com/vi/' . $hash . '/' . $size . '.jpg';
	}
	
	
	
	/**
	* Valida se uma URL é de um filme do Youtube
	* @param string $link URL do Youtube
	*/
	public static function valida_url_youtube($link)
	{
	      $valida_url = (bool) preg_match("#^http(s)?://[a-z0-9-_.]+\.[a-z]{2,4}#i",$link);
	      $valida_dominio1 = strpos($link,'youtube.com')!==false ? true : false;
	      $valida_dominio2 = strpos($link,'youtu.be')!==false ? true : false;
	      $valida_hash = self::get_hash_video($link)=='' ? false : true; 
	      
	      $retorno = $valida_url && ($valida_dominio1 || $valida_dominio2) && $valida_hash ? true : false;
	...
	/**
	* Retorna apenas o hash do vídeo do Youtube 
	 * @param string $link URL do Youtube
	*/
	public static function get_hash_video($link)
	{
	      $pattern1 = '#(?<=v=)[a-zA-Z0-9-]+(?=&)|(?<=v\/)[^&\n]+|(?<=v=)[^&\n]+|(?<=youtu.be/)[^&\n]+#';
	      $pattern2 = '#(?<=v=)[a-zA-Z0-9-]+(?=&)|(?<=v/)[^&\n]+(?=\?)|(?<=v=)[^&\n]+|(?<=youtu.be/)[^&\n]+#';
	      
	      if(preg_match($pattern1, $link, $matches)>0)
	      {
	            $retorno = $matches[0];
	      }
	      else 
	      {
	            $retorno = preg_match($pattern2, $link, $matches)>0 ? $matches[0] : '';
	      }
	...
	/**
	 * Cria os links para os endereços que estiverem no corpo do texto e devolve o texto com os links
	 * @param string $texto Texto original
	 * @param string $target Valor do atributo 'target' dos links a serem criados 
	 */
	public static function cria_links($texto='', $target='')
	{
		return preg_replace( '@(?<![.*">])\b(?:(?:https?|ftp|file)://|[a-z]\.)[-A-Z0-9+&#/%=~_|$?!:,.]*[A-Z0-9+&#/%=~_|$]@i', '<a href="\0" target="' . $target . '">\0</a>', $texto);		
	}

	
	/**
	 * Retorna a URL absoluta que força o download do arquivo e o renomeia (se for passado o parâmetro $nome_original)
	 * @param string $url URL http do arquivo
	 * @param string $nome_original Nome original do arquivo (será salvo com este nome na máquina do visitante)
	 */
	public static function get_link_download($url, $nome_original='')
	{
		$retorno = '';
		
		if($url!==null)
		{
			$string_original = $nome_original=='' ? '' : '&arquivo_orig=' . $nome_original;
			$retorno = DIR_HTM_ROOT . 'download.php?arquivo=' . $url . preg_replace('/[ ]+/', '-', strtolower(Utils::remove_acentos($string_original)));
		}
	...
	/**
	 * Limpa os valores nulos do vetor, trocando por string vazias
	 * @param array $vetor Vetor 
	 */
	public static function limpa_nulos($vetor)
	{
	...
	/**
	 * Retorna o ID de uma página no Facebook
	...
	/**
	 * Copia um arquivo de uma URL para o servidor, na pasta destino 
	 * @param string $url URL da imagem
	 * @param string $destino caminho completo da pasta destino do arquivo 
	 */
	public static function store_from_url($url, $destino)
	{
		return file_put_contents($destino, file_get_contents(str_replace('https://', 'http://', $url)));
	}
		
	
	/**
	 * Retorna a extensão de uma imagem (pelo tipo real da mesma)
	 * @param string $filename caminho físico para a imagem
	 */
	public static function get_image_extension($filename)
	{
		...
		{
			$extensao = strtolower(image_type_to_extension($image_info[2]));
			if (!$extensao) 
			{
				return false;
			}
		...
		$encurta = array('jpeg' => 'jpg', 'tiff' => 'tif',);
		
		return str_replace('.', '', strtr($extensao, $encurta));
	}
